...
numbers = [int(number) for number in almost_numbers]
greater_nums = [number for number in numbers if int(number) > 9]
print(greater_nums)
...
longer_than_eleven = [name for name in full_names if len(name) > 11]
surname = [name.split(" ")[1] for name in longer_than_eleven]
string_surname = " ".join(surname)
string_surname = string_surname.replace(" ", ", ")
